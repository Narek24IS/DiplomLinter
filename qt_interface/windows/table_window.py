...
from qt_interface.db.helpers import get_columns
from qt_interface.db.pgsql_connection import select, update
from qt_interface.interface.table_window_ui import Ui_TableWindow
from qt_interface.settings import settings
from qt_interface.windows.add_row_dialog import AddRowDialog
...
class TableWindow(QMainWindow, Ui_TableWindow):
    def __init__(self, main_table: str,
        ...
        self.setWindowTitle(
            f"Окно таблицы {main_table.capitalize().replace('_', ' ')}")
        ...
        horizontal_headers = headers.copy()
        for column in self.columns:
            headers.append(column)
            column = column.capitalize().replace('_', ' ').replace('.', ' ')
            horizontal_headers.append(column)
    ...
    def load_data(self):
        ...
        for i, row in enumerate(data):
            ...
            if self.btn_func:
                ...
                pv=self._get_value_by_column(row, self.main_pk)
                btn.clicked.connect(
                    lambda _, parent_window=self, row_num=i,
                           pk_value=pv: self.btn_func(parent_window,
                                                                                                row_num, pk_value))
...
if __name__ == "__main__":
    ...
    user_id = 1
    window = TableWindow(main_table="users", user_id=1,
