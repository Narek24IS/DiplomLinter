...
function BuildFileList() {
  ...
  debug "Building the list of files and directories to check"
  ...
  if [ -n "${STDOUT_BUILD_FILE_LIST}" ]; then
    debug "Command output when building the file list:\n------\n${STDOUT_BUILD_FILE_LIST}\n------"
  else
  ...
  if [ -n "${STDERR_BUILD_FILE_LIST}" ]; then
    debug "Stderr when building the file list:\n------\n${STDERR_BUILD_FILE_LIST}\n------"
  else
  ...
  # Footer print #
  ################
  debug "Successfully gathered list of files..."
}
